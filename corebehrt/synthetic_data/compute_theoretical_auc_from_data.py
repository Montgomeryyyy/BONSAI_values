...
def detect_switches_in_data(
    data: pd.DataFrame, lab_code: str = "S/LAB1"
) -> pd.DataFrame:
    """
        ...
        patient_switches.append(
            {
                ...
                "switch_ratio": switches / (len(values) - 1)
                if len(values) > 1
                else 0.0,
            }
        )
...
def compute_switch_based_auc(
    data: pd.DataFrame, positive_diags: list = ["S/DIAG_POSITIVE"]
) -> float:
    """
...
def compute_variance_based_auc(
    data: pd.DataFrame, positive_diags: list = ["S/DIAG_POSITIVE"]
) -> float:
    """
...
def compute_sequence_complexity_auc(
    data: pd.DataFrame, positive_diags: list = ["S/DIAG_POSITIVE"]
) -> float:
    """
...
def compute_theoretical_auc_from_data(
    data: pd.DataFrame,
    ...
    try:
        ...
        results["sequence_complexity_auc"] = compute_sequence_complexity_auc(
            data, positive_diags
        )
        ...
        if len(lab_data) > 0:
            positive_patients = set()
            for diag in positive_diags:
                positive_patients.update(
                    data[data["code"] == diag]["subject_id"].unique()
                )
...
def print_theoretical_auc_results(
    data: pd.DataFrame,
    positive_diags: list = ["S/DIAG_POSITIVE"],
    lab_code: str = "S/LAB1",
) -> None:
    ...
    if len(lab_data) > 0:
        print(
            f"  Lab value range: {lab_data['numeric_value'].min():.3f} - {lab_data['numeric_value'].max():.3f}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compute theoretical AUC from actual data"
    )
    parser.add_argument(
        "--data_file",
        type=str,
    ...
    parser.add_argument(
        "--positive_diags",
        nargs="+",
        default=["S/DIAG_POSITIVE"],
        help="List of positive diagnosis codes",
    )
    parser.add_argument(
        "--lab_code", type=str, default="S/LAB1", help="Lab code to analyze"
    )
