...
def get_positive_patients_from_timestamps(
    patient_info: pd.DataFrame, pos_col: str
) -> pd.DataFrame:
    """
    ...
    if pos_col not in patient_info.columns:
        print(
            f"Warning: Column '{pos_col}' not found in patient_info. No patients will be marked as positive."
        )
    ...
    print(
        f"Found {positive_count} positive patients out of {total_count} total patients ({positive_count / total_count * 100:.1f}%)"
    )

    return patient_info


def select_patients_for_labs(
    patient_info: pd.DataFrame, percent_labs: float
) -> pd.DataFrame:
    """
    ...
    selected_indices = np.random.choice(
        patient_info.index, size=num_patients_with_labs, replace=False
    )
    ...
    print(
        f"Selected {num_patients_with_labs} patients ({percent_labs * 100:.1f}%) for lab insertion"
    )

    return patient_info


def generate_lab_value_for_patient(
    is_positive: bool, low_mean: float, high_mean: float, std: float
) -> float:
    """
...
def generate_lab_concepts_for_selected_patients(
    ...
    for _, patient in patients_with_labs.iterrows():
        ...
        for i in range(num_labs):
            lab_value = generate_lab_value_for_patient(
                is_positive, low_mean, high_mean, std
            )
...
def generate_timestamps_for_labs(
    patient_info: pd.DataFrame, concepts_data: pd.DataFrame
) -> List[pd.Timestamp]:
    """
    ...
    for _, row in concepts_data.iterrows():
        ...
        if len(patient_match) == 0:
            raise ValueError(
                f"Patient {pid} not found in patient_info. This should not happen."
            )
        ...
        if pd.isna(birthdate) or birthdate is None:
            print(
                f"Warning: Patient {pid} has no birthdate or NaT birthdate. Skipping patient."
            )
        ...
        if time_diff <= 0:
            ...
            max_offset = max(
                0, time_diff - (lab_index * 30 * 24 * 3600)
            )  # Leave room for lab spacing
            random_seconds = np.random.randint(0, int(max_offset))
            timestamp = (
                start_date
                + pd.Timedelta(seconds=random_seconds)
                + pd.Timedelta(days=lab_index * 30)
            )
...
def calculate_theoretical_auc_improvement(
    ...
    lab_patients_with_labs = patients_with_labs.intersection(
        positive_patients.union(negative_patients)
    )
    ...
    if lab_auc > 0.5:
        ...
        combined_auc = 1 - (1 - baseline_auc) * (1 - lab_auc) / (
            1 - baseline_auc * lab_auc
        )
    else:
    ...
    return {
        ...
        "improvement_percentage": (auc_improvement / max_improvement) * 100
...
def main():
    ...
    print(
        f"  - Gaussian distribution for negative patients: mean={args.low_mean}, std={args.std}"
    )
    print(
        f"  - Gaussian distribution for positive patients: mean={args.high_mean}, std={args.std}"
    )
    ...
    if len(data) > 0:
        ...
        negative_patients = set(
            patient_info[~patient_info["is_positive"]]["subject_id"]
        )
        ...
        if total_negative > 0:
            ...
            print(
                f"Negative patients with labs: {negative_with_labs}/{total_negative} ({negative_percentage:.1f}%)"
            )
        else:
    ...
    if args.baseline_auc is not None and len(data) > 0:
        ...
        if "error" in auc_results:
            ...
            print(
                f"Maximum possible improvement: {auc_results['max_possible_improvement']:.3f}"
            )
            print(
                f"Improvement percentage: {auc_results['improvement_percentage']:.1f}% of maximum possible"
            )
            print(
                f"Patients with labs: {auc_results['patients_with_labs']}/{auc_results['total_patients']}"
            )
